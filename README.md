...
# Skills
- Linux administration    -  I am using Fedora as my main PC, whilst Pi server is running on a Trixie OS (Debian).
...
- Docker                  -  Using containers to run various services on Pi Server. Docker-compose files and .env used for initializing the containers, whilst volumes stored on external HDD.
...
- DNS / Pi-hole           -  Due to the limitation of the ISP router (only DNS Forwarding is available), Local DNS requires static IP addressing on the end hosts.
- HTTPS / PKI             -  Ngnix Proxy Manager is used to ensure secure SSL connection to the local services runnning via Docker. I created CA, Intermediate CA and SSL certificates accordingly.
...
- Git                     -  Repositories are stored on the Pi Server and pushed to Github's private repos.
